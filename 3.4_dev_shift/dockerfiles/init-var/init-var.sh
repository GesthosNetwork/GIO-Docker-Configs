...
for server in dbgate dispatch gameserver gateserver muipserver multiserver nodeserver oaserver pathfindingserver tothemoonserver; do
  [ -f server/$server/conf/$server.xml.tmpl ] || continue
